...
class MediaDownloader:
    ...
    def download(self) -> dict | None:
        ...
        if is_offline and is_final_attempt:
            print(f"[FIL] SKIP {self.download_info.filename} domain_offline")
        ...
        # Attempt to download the file with retries
        print(f"[FIL] START {formatted_filename}")
        try:
        ...
        if failed_download:
            print(f"[FIL] FAIL {formatted_filename}")
            return self._handle_failed_download(is_final_attempt=is_final_attempt)

        print(f"[FIL] DONE {formatted_filename}")
