...
class LiveManager:
    ...
    def _log_results_summary(self) -> None:
        ...
        # Structured summary for webui
        completed = self.summary_manager.get_result_count(TaskResult.COMPLETED)
        failed = self.summary_manager.get_result_count(TaskResult.FAILED)
        skipped = self.summary_manager.get_result_count(TaskResult.SKIPPED)
        et = self._compute_execution_time()
        print(f"[SUM] completed={completed} failed={failed} skipped={skipped} time={et}")
